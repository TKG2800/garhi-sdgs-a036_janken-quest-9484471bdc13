...
public static class ConstantsClassCreator
{
  ...
  private const string DirectoryPath =  "Assets/UnityAds/Scripts/";
  ...
  public static void Create<T> (string className, string classInfo, Dictionary<string, T> valueDic)
  {
    ...
    if(typeof(T) == typeof(string)){
      ...
      Debug.Log (className + Extension +"の作成に失敗しました.想定外の型" + typeof(T).Name  + "が入力されました");
      return;
    }
    ...
    //書き出し、ファイル名はクラス名.cs
    string exportPath = DirectoryPath + className + Extension;
    ...
    Debug.Log (exportPath + "の作成が完了しました");
  }
